---
name: dbt-linear-accelerator
description: "Accelerate dbt development from Linear issues. Reads a Linear issue, identifies the relevant dbt repo, creates a branch, and implements the required dbt model changes conversationally. Use when: analytics engineer has a Linear issue for dbt work, dbt issue, implement Linear issue, work on dbt issue, dbt from Linear, pick up issue. Triggers: dbt linear, dbt issue, linear issue dbt, Linear ticket dbt, pick up issue, work on issue, implement issue."
---

# dbt Linear Accelerator

Accelerates analytics-engineering work by reading Linear issues and implementing dbt model changes in the correct repository with a proper git workflow.

> Run the **dbt-coco-setup** skill once before using this skill. It installs the required MCP servers (dbt + Linear), authenticates GitHub, and fills in the Workflow Context block below.

<!-- USER_WORKFLOW_CONTEXT:START -->
## Workflow Context

_Not configured yet. Run the `dbt-coco-setup` skill to populate this block. Until then, ask the user for any of the following you need: Linear team key(s), workflow state names, whether they use Projects/Cycles, dbt flavor (Core/Cloud), dbt project path, whether the default target is dev or prod, default branch, branch-naming and commit conventions, and PR method (gh CLI vs push-only)._
<!-- USER_WORKFLOW_CONTEXT:END -->

## Companion skills (dbt Labs official)

This accelerator **orchestrates** the issue-to-PR workflow. It does **not** re-implement dbt craft — it delegates to **dbt Labs' official, maintained skills** ([`dbt-labs/dbt-agent-skills`](https://github.com/dbt-labs/dbt-agent-skills)), installed by `dbt-coco-setup`. Prefer these skills whenever they're available; they're purpose-built for dbt and kept current by the dbt team:
...
Step 0: Load workflow context + memory
  ↓
Step 1: Read & Understand the Linear Issue       ⚠️ STOP
  ↓
...
Step 7: Commit, Push & Open PR                    ⚠️ STOP (commit message)
  ↓
Done → PR created and linked to the Linear issue
```
...
1. Read the **Workflow Context** block above. Use its values (team keys, workflow states, projects/cycles usage, dbt flavor, project path, target safety, branch/commit conventions, PR method) throughout — don't re-ask for anything already there. If the block is still the "Not configured yet" placeholder, tell the user to run `dbt-coco-setup`, or gather the missing details inline.
2. Check `/memories` for anything saved about this dbt repo from prior sessions — e.g. test syntax conventions (`arguments:` wrapper), the confirmed dev/prod target, naming patterns, or downstream systems (semantic views, Cortex Agents, dashboards) that tend to need updating. Apply what you find; verify it still matches the current repo before relying on it.

### Step 1: Read & Understand the Linear Issue

**Goal:** Fully understand the request before touching code — and gather the context a vague issue leaves out. A one-line issue is common; do the digging an analytics engineer would do rather than guessing.

**Actions:**

1. **Get the issue.** Ask for the identifier/URL (e.g. `ENG-123`) or accept one given; resolve partial references with `search_issues` (scoped to the team key(s) from context).
2. **Fetch the issue *and its context*** via the Linear MCP:
   - The issue: title, description, acceptance criteria, comments, labels, project, cycle, priority, status, due date.
   - **Linked & parent context:** the parent issue, sub-issues, related/linked issues, and the **project (and its docs/updates)** — requirements often live there, not in the issue body.
   - **Other doc sources:** if the issue links a Confluence page, Notion doc, Google Doc, or other spec/metric definition and a matching MCP or fetchable link is available, **read it**; otherwise ask the user to paste the relevant definition.
3. **Run a completeness check** — do you actually have enough to build it correctly? List it as known vs unknown:
   ...
   - Acceptance criteria: how success is measured (expected row count, sample values, edge cases).
...
**After the user confirms:** Move the issue to the active "in progress" state from the Workflow Context, if the Linear MCP exposes a state-update tool. If not, tell the user what to update manually.
...
1. **Auto-generate** the branch name from the convention in context (default `feature/<ISSUE-ID>-<slug>`). Build `<slug>` from the issue title: lowercase it, replace every run of non-alphanumeric chars with a single hyphen, trim leading/trailing hyphens, and truncate so the whole branch name is ≤ 60 chars. Example: `ENG-123` "Add Customer Lifetime Value" → `feature/ENG-123-add-customer-lifetime-value`.
...
**Goal:** Map issue requirements to specific dbt changes.

**Delegate:** for model planning use `using-dbt-for-analytics-engineering` (plan backwards from output, discover data); for precise downstream detection use `using-dbt-state` (`dbt ls --select state:modified+`); if the issue **renames/removes/retypes a column on a model with consumers** (a breaking change), use `working-with-dbt-mesh` to version it instead of editing in place.
...
**⚠️ STOP**: Get user approval on the plan before implementing. Ask about business logic, naming, materialization, whether downstream systems update now or in a follow-up issue, and confirm the **deployment approach** (full-refresh / timing vs the scheduled job) if flagged.

### Step 5: Implement dbt Models

**Goal:** Write/modify the models to fulfill the issue. **This step is conversational.**

**Delegate:** use `using-dbt-for-analytics-engineering` for the SQL/model work (`ref()`/`source()`, CTEs, YAML docs, `dbt show` profiling) and `adding-dbt-unit-test` for tests; use `building-dbt-semantic-layer` if the issue involves MetricFlow metrics. Keep showing each change and confirming business logic (this skill's conversational rule stands).
...
4. **Update** schema/test YAML — docs + meaningful test coverage (match existing patterns, incl. whether tests use dbt's `arguments:` syntax):
   ...
   - `accepted_values` / range / `not_null` on the columns the issue cares about.
...
3. **Validate the output against the acceptance criteria** (dev target only — compiling is not enough):
   ...
   - The numbers **match what the issue asked for** (e.g. the metric reconciles). Use `dbt show` / targeted queries.
...
**Goal:** Commit the work and open a PR linked to the issue.
...
3. **Suggest** a commit message using the convention from context (default conventional commits): `feat(<area>): <description> [<ISSUE-ID>]`.

**⚠️ STOP**: Ask the user to confirm or modify the commit message before committing.

4. **Commit** only after confirmation.
5. **Present a change summary** linking work to the issue:
   ```
...
6. **Push and open the PR** according to the PR method in context.
   ...
   - If PR method is **gh CLI**:
     ```bash
     gh pr create --title "<ISSUE-ID>: <short description>" --body "$(cat <<'EOF'
     ## Summary
...
7. **Comment on the Linear issue** with the PR link (if the Linear MCP exposes a comment tool) and move it to the review state from context.
8. Present the PR URL to the user.

### Closing: save durable learnings (optional)

If you learned something durable about this repo that isn't already captured — e.g. a test-syntax quirk, the confirmed dev/prod target, a naming convention, or a downstream system that needed updating — **offer** to save it to `/memories` so future runs start smarter. Only save with the user's okay; don't record ephemeral task details or secrets.

## Stopping Points

- ✋ Step 1: after summarizing the issue — confirm understanding
...
- Requires the **Linear MCP** and a local **dbt project** (plus the **dbt MCP**). Run `dbt-coco-setup` first, which also installs dbt Labs' official skills.
- If the Linear or dbt MCP tools aren't available in the session (e.g. just installed), run `cortex mcp start` (or `cortex mcp reconnect` for a stuck server) **yourself** to connect them live, then retry — don't ask the user to restart. A reload is a last resort only on older CoCo builds.
...
- Linear MCP tool names vary by version (e.g. `get_issue`, `search_issues`, state-update and comment tools); use the available equivalents.
...
- PR created (or compare URL provided) with change summary and Linear issue link
- Linear issue moved/commented when MCP support allows
