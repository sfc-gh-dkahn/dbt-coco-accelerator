---
name: dbt-jira-accelerator
description: "Accelerate dbt development from Jira tickets. Reads a Jira ticket, identifies the relevant dbt repo, creates a branch, and implements the required dbt model changes conversationally. Use when: analytics engineer has a Jira ticket for dbt work, dbt ticket, implement Jira ticket, work on dbt ticket, dbt from Jira, pick up ticket. Triggers: dbt jira, dbt ticket, jira ticket dbt, pick up ticket, work on ticket, implement ticket."
---

# dbt Jira Accelerator

Accelerates analytics-engineering work by reading Jira tickets and implementing dbt model changes in the correct repository with a proper git workflow.

> Run the **dbt-coco-setup** skill once before using this skill. It installs the required MCP servers (dbt + Atlassian/Jira), authenticates GitHub, and fills in the Workflow Context block below.

<!-- USER_WORKFLOW_CONTEXT:START -->
## Workflow Context

_Not configured yet. Run the `dbt-coco-setup` skill to populate this block. Until then, ask the user for any of the following you need: Jira site URL, project key(s), workflow state names, dbt flavor (Core/Cloud), dbt project path, whether the default target is dev or prod, default branch, branch-naming and commit conventions, and PR method (gh CLI vs push-only)._
<!-- USER_WORKFLOW_CONTEXT:END -->

## Companion skills (dbt Labs official)

This accelerator **orchestrates** the ticket-to-PR workflow. It does **not** re-implement dbt craft — it delegates to **dbt Labs' official, maintained skills** ([`dbt-labs/dbt-agent-skills`](https://github.com/dbt-labs/dbt-agent-skills)), installed by `dbt-coco-setup`. Prefer these skills whenever they're available; they're purpose-built for dbt and kept current by the dbt team:
...
Step 0: Load workflow context + memory
  ↓
Step 1: Read & Understand the Jira Ticket        ⚠️ STOP
  ↓
...
Step 7: Commit, Push & Open PR                    ⚠️ STOP (commit message)
  ↓
Done → PR created and linked to the Jira ticket
```
...
### Step 1: Read & Understand the Jira Ticket

**Goal:** Fully understand the request before touching code — and gather the context a vague ticket leaves out. A one-line ticket is common; do the digging an analytics engineer would do rather than guessing.
...
1. **Auto-generate** the branch name from the convention in context (default `feature/<TICKET-KEY>-<slug>`). Build `<slug>` from the ticket summary: lowercase it, replace every run of non-alphanumeric chars with a single hyphen, trim leading/trailing hyphens, and truncate so the whole branch name is ≤ 60 chars. Example: `DE-1234` "Add Customer Lifetime Value" → `feature/DE-1234-add-customer-lifetime-value`.
...
**Goal:** Map ticket requirements to specific dbt changes.

**Delegate:** for model planning use `using-dbt-for-analytics-engineering` (plan backwards from output, discover data); for precise downstream detection use `using-dbt-state` (`dbt ls --select state:modified+`); if the ticket **renames/removes/retypes a column on a model with consumers** (a breaking change), use `working-with-dbt-mesh` to version it instead of editing in place.
...
**⚠️ STOP**: Get user approval on the plan before implementing. Ask about business logic, naming, materialization, whether downstream systems update now or in a follow-up ticket, and confirm the **deployment approach** (full-refresh / timing vs the scheduled job) if flagged.

### Step 5: Implement dbt Models

**Goal:** Write/modify the models to fulfill the ticket. **This step is conversational.**

**Delegate:** use `using-dbt-for-analytics-engineering` for the SQL/model work (`ref()`/`source()`, CTEs, YAML docs, `dbt show` profiling) and `adding-dbt-unit-test` for tests; use `building-dbt-semantic-layer` if the ticket involves MetricFlow metrics. Keep showing each change and confirming business logic (this skill's conversational rule stands).
...
4. **Update** schema/test YAML — docs + meaningful test coverage (match existing patterns, incl. whether tests use dbt's `arguments:` syntax):
   ...
   - `accepted_values` / range / `not_null` on the columns the ticket cares about.
...
3. **Validate the output against the acceptance criteria** (dev target only — compiling is not enough):
   ...
   - The numbers **match what the ticket asked for** (e.g. the metric reconciles). Use `dbt show` / targeted queries.
...
**Goal:** Commit the work and open a PR linked to the ticket.
...
3. **Suggest** a commit message using the convention from context (default conventional commits): `feat(<area>): <description> [<TICKET-KEY>]`.

**⚠️ STOP**: Ask the user to confirm or modify the commit message before committing.

4. **Commit** only after confirmation.
5. **Present a change summary** linking work to the ticket:
   ```
   ## Change Summary — <TICKET-KEY>
...
6. **Push and open the PR** according to the PR method in context.
   ...
   - If PR method is **gh CLI**:
     ```bash
     gh pr create --title "<TICKET-KEY>: <short description>" --body "$(cat <<'EOF'
     ## Summary
     <change summary from sub-step 5>

     ## Jira Ticket
     <JIRA_SITE_URL>/browse/<TICKET-KEY>
...
- ✋ Step 1: after summarizing the ticket — confirm understanding
...
- Requires the **Atlassian/Jira MCP** and a local **dbt project** (plus the **dbt MCP**). Run `dbt-coco-setup` first, which also installs dbt Labs' official skills.
...
- PR created (or compare URL provided) with change summary and Jira link
- Jira ticket transitioned/commented when MCP support allows
